...
## How Autopilot protects a workspace

**Sign-in.** OAuth (Google, GitHub) through Auth.js with database sessions in httpOnly cookies.
Autopilot stores no passwords. Demo sign-in is off unless `ALLOW_DEMO_LOGIN=true`, and the server
...
**Roles.** Members can plan sprints and run agents. Writing to Jira, Xray or Bitbucket and changing
integrations need the owner or admin role.
...
lower coverage or change assertions.
