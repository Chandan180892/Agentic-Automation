/* Generated from src/lib/agents/rulebook.ts by npm run build:pages. Do not edit. */
"use strict";
var Rulebook = (() => {
  ...
  var hasAssertion = (body) => /\bexpect(\.poll|\.soft)?\s*\(|\bassert\w*\s*\(|\.should\(/.test(body);
  var RULES = [
    // --------------------------------------------------------------- waiting --
    {
      id: "no-fixed-waits",
    ...
    },
    {
      ...
      check: {
        ...
        fix: (line) => {
          ...
          const assertion = { isVisible: "toBeVisible", isHidden: "toBeHidden", isEnabled: "toBeEnabled", isDisabled: "toBeDisabled", isChecked: "toBeChecked" }[method];
    ...
    },
    {
      ...
      check: {
        kind: "test-block",
        test: (_name, body) => /\brequest\.(get|post|put|patch|delete|fetch)\(/.test(body) && /\.status\(\)|toBeOK\(\)/.test(body) && !/\.json\(\)|\.text\(\)|toMatchObject|toHaveProperty|toEqual\(\s*\{|toContain|schema|\.body/.test(body)
      }
    },
    // ------------------------------------------------------------------ data --
    {
    ...
    // ------------------------------------------------- guidance only (prompts) --
    {
      ...
      why: "UI tests are 10\u2013100\xD7 slower and flakier than API tests for the same rule; an inverted pyramid is the most common reason suites become unmaintainable.",
  ...
  function rulebookPrompt() {
    const byLayer = (l) => RULES.filter((r) => r.layer === l);
    const line = (r) => `- ${r.title}${r.severity === "block" ? " [blocking]" : ""}: ${r.instead}`;
    return [
  ...
  var isTestFile = (path) => /\.(spec|test)\.[cm]?[jt]sx?$/.test(path);
  var isCode = (path) => /\.[cm]?[jt]sx?$/.test(path);
  ...
  function qualityGate(files) {
    ...
    const out = files.map((f) => {
      ...
      lines.forEach((raw, idx) => {
        ...
        for (const r of RULES) {
          ...
          const fixedLine = r.check.fix ? r.check.fix(line) : line;
          const fixed2 = r.check.fix !== void 0 && fixedLine !== line;
          findings.push({ rule: r.id, title: r.title, severity: r.severity, path: f.path, line: idx + 1, text: raw.trim().slice(0, 160), fixed: fixed2 });
          if (fixed2) line = fixedLine;
        }
    ...
    return {
      ...
      blocking,
      warnings,
      summary: findings.length === 0 ? `${checked} rules checked: clean.` : `${checked} rules checked: ${fixed} fixed automatically, ${blocking} blocking, ${warnings} warning(s).`
    };
  }
  return __toCommonJS(rulebook_exports);
})();
